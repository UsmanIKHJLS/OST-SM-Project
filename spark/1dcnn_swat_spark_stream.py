...
# Initialize Spark
spark = SparkSession.builder.appName("SWaT_RealTime_Anomaly_Prediction").getOrCreate()
...
MODEL_PATH = os.getenv("MODEL_PATH") + "/1dcnn"
model = TFSMLayer(f"models/{MODEL_PATH}", call_endpoint='serving_default')

# Load model correctly
# model = load_model(f"models/{MODEL_PATH}/swat_1dcnn_model.h5")
...
casted_df = parsed_df.select(
    *(col(c).cast("float").alias(c) if c not in ["Normal/Attack", "Label"] else col(c) for c in parsed_df.columns)
)

# Map Labels
casted_df = casted_df.withColumn("Label", when(col("Normal/Attack") == "Normal", 0).otherwise(1))

# Remove "Normal/Attack" column and ensure numeric features
casted_df = casted_df.drop("Normal/Attack")

# Feature Assembly
feature_cols = [col for col in casted_df.columns if col not in ["Label"]]
assembler = VectorAssembler(inputCols=feature_cols, outputCol="features")
feature_df = assembler.transform(casted_df)

# Predict Function with Dynamic Threshold
